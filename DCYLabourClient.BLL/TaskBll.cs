...
namespace DCYLabourClient.BLL
{
    public class TaskBll
    {
        ...
        public List<TaskInfo> GetTaskList(string stucardnum)
        {
            DataTable dt = tdal.GetTaskList(stucardnum);
            if (dt.Rows.Count == 0)
                return null;
            else
            {
                ...
                for(int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    task.Fandi = int.Parse(dt.Rows[i]["Fandi"].ToString() == "" ? "0" : dt.Rows[i]["Fandi"].ToString());
                    task.Zhengdi = int.Parse(dt.Rows[i]["Zhengdi"].ToString() == "" ? "0" : dt.Rows[i]["Zhengdi"].ToString());
                    task.Celiang = int.Parse(dt.Rows[i]["Celiang"].ToString() == "" ? "0" : dt.Rows[i]["Celiang"].ToString());
                    task.CuoShi = int.Parse(dt.Rows[i]["CuoShi"].ToString() == "" ? "0" : dt.Rows[i]["CuoShi"].ToString());
        ...
        public TaskFinishInfo GetFinishInfo(int taskid)
        {
            DataTable dt = tdal.GetFinishInfo(taskid);
            if (dt.Rows.Count == 0)
                return null;
            else
            {
